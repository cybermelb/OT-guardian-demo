...
// Since Vercel doesn't expose environment variables (like __firebase_config) 
// to client-side JS automatically for static deployments, we hardcode the configuration 
// provided by the user to guarantee initialization.
...
// Critical: Mapping the user UIDs you provided to roles.
const USER_ROLES_MAP = {
    ...
    // Default mock role for demonstration
    'mock-analyst-uid': 'analyst',
};
...
const AdminPanel = ({ userId, userRole, controls, setControls }) => {
    ...
    const handleDeleteControl = async (controlId, controlName) => {
        ...
        if (!window.confirm(`Are you sure you want to delete the control: ${controlName}? This action cannot be undone.`)) {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
            if (currentUser) {
                ...
                try {
                    // Check for custom auth token (if running in Canvas)
